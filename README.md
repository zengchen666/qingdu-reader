...
| 书内全文搜索 | Ctrl+F 搜索全书，结果显示所属章节 + 命中上下文，点击跳转并高亮命中词 | **已完成（v0.2.0）** |
...
下载站给的 TXT 里，书名可能出现在三个地方，可靠性差很多：

| 来源 | 例子 | 可靠性 |
|---|---|---|
| 文件头里的 `书名：xxx` | `书名：斗破苍穹` | 最可靠 |
| 文件头里**用书名号标出来**的短行 | `《元尊》`、`《全职高手》（精校版全本）作者蝴蝶蓝`、`『武动乾坤/作者:天蚕土豆』` | 可靠 |
| 文件名 | `《斗破苍穹》（精校无删减版）作者：天蚕土豆.txt` | 兜底 |

优先级是**内容 > 文件名**：批量下载的文件经常被统一改名成 `01.txt`、`02.txt`，名字里根本没有书名。
早先只认"书名："这一种写法，四本真实语料里只有一本能认出书名，其余三本在书架上就显示成 `01 / 02 / 04`。

判断"文件头里哪一行是书名"只用两个信号：**书名号**（作者留下的"这是书名"标记）和**行长**
（书名都短；正文里即便引述书名，那一行也被句子的其余部分撑长了）。
`『状态:已完结』` 这类元数据行也长着"短行 + 书名号"的样子，靠"书名里不会出现冒号和句末标点"把它挡掉。

拿到候选后还要清洗：括号注释、`作者xxx` 尾巴、残留的分隔符、外层书名号。
这几步**文件名和文件头共用同一套逻辑**，因为它们遇到的是同一类噪声。
其中书名号要**头尾各剥一次** —— 开头那次是给 `『武动乾坤/作者:天蚕土豆』` 用的
（先剥壳，否则"作者尾巴"会把闭书名号一起吃掉，剩下 `『武动乾坤` 这种半个壳的结果）；
末尾那次是给 `《全职高手》（精校版全本）作者蝴蝶蓝` 用的（剪掉尾巴之后，书名号才"成对"露出来）。

---

### 13. 中文全文搜索：FTS5 内置的分词方案全都不能用

给中文做全文搜索，最省事的想法是"交给 SQLite FTS5 的内置分词器"。实测下来两条路都堵死：

| 方案 | 两字查询（石符 / 林动 / 小貂） | 三字查询（青阳镇 / 聚气散） |
|---|---|---|
| FTS5 默认 `unicode61` | ❌ 全 0 | ❌ 全 0 |
| FTS5 内置 `trigram`（3.34+） | ❌ **全 0** | ✅ 命中 |
| **自己做的中文二字滑窗（bigram）** | ✅ 命中 | ✅ 命中 |

`unicode61` 把连续的一串汉字当成**一个** token，子串自然搜不到；
`trigram` 按三个字滑窗，于是**两字查询必然匹配不到任何 token** ——
而中文里两字词、两字人名（林动、萧炎、叶修）恰恰是最常见的搜法。
网上不少「FTS5 中文搜索」的教程直接上 trigram，踩的就是这个坑。

所以分词自己做：只认汉字（标点丢掉，省 10% 体积），二字滑窗，ASCII 字母数字整段保留。

**bigram 有个绕不开的副作用：它保证两个 token 都出现，不保证相邻。**
「云岚山中有个岚宗派」会被搜「云岚宗」命中。而能要求相邻的短语查询在
`detail='none'` 下不可用（实测报错），所以查询之后**必须**拿原文再做一次精确子串校验。
好在展示结果本来就要读原文生成摘要，这一刀几乎是白送的 ——
也因此选了"不存原文"的表结构：索引体积 69 MB → **29 MB**。

真书语料实测（4 本共 6100 章）：建索引 3.8~6.4 秒/本，查询 16~104 ms，
19 个查询词**候选 == 精确（零假阳性）**。完整数据见
[`docs/2026-09-22-fulltext-search-report.md`](docs/2026-09-22-fulltext-search-report.md)。

⚠️ 这里还有个**验收口径**的坑值得记一笔：查询集满条数上限就停，
所以「候选数 − 命中数」在截断时是"**没查的**"，不是"**查了不合格的**"。
第一版验收脚本正是把它当成了假阳性，把「萧炎」报成有 1322 个假阳性 ——
数字看着合理、结论完全相反。现在 `SearchResult` 用 `truncated` / `unchecked()`
把这两件事在类型上分开，并有单元测试钉住。
...
| 2 | 书库管理：**书架 + 批量导入 + 封面占位已完成（v0.1.1）**；分组、书签备注、收藏夹监控、阅读时长待做 | **进行中** |
| 2.1 | 全文搜索：**SQLite FTS5 + 中文二字滑窗分词已完成（v0.2.0）**；跨书搜索待做 | **进行中** |
| 3 | 沉浸体验：全屏阅读、自动滚动、行距 / 双栏 | 计划中 |
| 4 | EPUB 支持：epub4j 解析、统一章节模型落地、图片处理 | 计划中 |
| 5 | 打包发布：jlink + jpackage、GitHub Release | **已完成（绿色版 + [v0.1.0](https://github.com/zengchen666/qingdu-reader/releases/tag/v0.1.0) / [v0.1.1](https://github.com/zengchen666/qingdu-reader/releases/tag/v0.1.1) / [v0.2.0](https://github.com/zengchen666/qingdu-reader/releases/tag/v0.2.0) Release；`.msi` 安装包待做）** |
